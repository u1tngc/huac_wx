...
def get_kyosho(timeKbn, currentDateTime, fileName):
    fileName = get_kyosho_asas(timeKbn, currentDateTime, fileName)
    fileName = get_kyosho_kousou(timeKbn, currentDateTime, fileName)
    return fileName

def get_kyosho_kousou(timeKbn, currentDate, fileName):
    ...
    while eof_flg <= 1:
        try:
            ...
            file = open("高層天気図(北半球500)_" + str(2 - eof_flg) + ".pdf","wb")
    ...
    response = requests.get(url_50)
    file = open("高層天気図(北半球500)_3.pdf","wb")
...
def get_kyosho_asas(timeKbn, currentDate, fileName):
    try:
        ...
        if timeKbn == 21:
            ...
            for ix1 in range(2):
                ...
                file = open("地上天気図_" + str(2 - ix1) + ".pdf","wb")
        ...
        file = open("fefe19.png","wb")
        fileName[5] = "fefe19.pdf"
        for chunk in response.iter_content(200000):
            file.write(chunk)    
        file.close()
        #pngファイルのpdf化
        image1 = Image.open('fefe19.png')
        im_pdf = image1.convert("RGB")
        im_pdf.save("fefe19.pdf")
        #pngファイルの削除
        os.remove('fefe19.png')
        return fileName
